...
export default function Settings() {
  const [form, setForm] = useState<any>({llm_provider:'kimi', library_path:'./library'});
  ...
  return <main style={{padding:20}}>
    ...
    <select value={form.llm_provider} onChange={(e)=>setForm({...form,llm_provider:e.target.value})}><option value="kimi">kimi</option><option value="openai">openai</option><option value="gemini">gemini</option></select>
    <input placeholder="Kimi API key" onChange={(e)=>setForm({...form,kimi_api_key:e.target.value})}/>
